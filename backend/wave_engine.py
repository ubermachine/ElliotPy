...
class DailyElliottWaveEngine:
    ...
    def run_adaptive_zigzag(self, min_move_mult: float = 1.0, start_idx: int = 0, end_idx: int = None) -> List[Pivot]:
        ...
        h_idx = start_idx + np.nanargmax(self._high_vals[start_idx:start_idx+initial_period])
        l_idx = start_idx + np.nanargmin(self._low_vals[start_idx:start_idx+initial_period])
    ...
    def score_impulse(self, pivots: List[Pivot]) -> float:
        ...
        if self._rsi_vals is not None:
            try:
                rsi_3 = self._rsi_vals[pivots[3].index]
                rsi_5 = self._rsi_vals[pivots[5].index]
    ...
    def _recursive_subdivide(self, parent_wave: WaveNode, default_min_move: float, depth: int):
        ...
        dynamic_move_mult = max(0.1, (parent_range_pct * 0.20) / np.nanmean(self._min_move_pct_vals[start_idx:end_idx]))
    ...
    def get_summary_engine_payload(self, primary_waves: List[WaveNode], alternate_waves: List[List[WaveNode]]) -> Dict[str, Any]:
        def build_count_data(waves: List[WaveNode]):
            ...
            if len(last_block) == 5:
                ...
                if self._macd_hist_vals is not None and not pd.isna(self._macd_hist_vals[pivs[-1].index]):
                    macd_val = self._macd_hist_vals[pivs[-1].index]
