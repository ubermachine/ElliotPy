...
class DailyElliottWaveEngine:
    ...
    def __init__(self, df: pd.DataFrame, use_log_scale: bool = False):
        ...
        # ⚡ Bolt: Caching pandas columns as numpy arrays via .values eliminates .iloc overhead in tight loops
        # Cache arrays for fast loop access
        self._closes = self.df['Close'].values
        self._highs = self.df['High'].values
        self._lows = self.df['Low'].values
        self._log_closes = self.df['Log_Close'].values
        self._log_highs = self.df['Log_High'].values
        self._log_lows = self.df['Log_Low'].values
        self._dates = self.df['Date'].values
        self._min_move_pcts = self.df['min_move_pct'].values
        self._atrs = self.df['ATR'].values
        self._rsis = self.df['RSI'].values if 'RSI' in self.df.columns else None
        self._macd_hists = self.df['MACD_Hist'].values if 'MACD_Hist' in self.df.columns else None
    ...
    def run_adaptive_zigzag(self, min_move_mult: float = 1.0, start_idx: int = 0, end_idx: int = None) -> List[Pivot]:
        ...
        h_idx = start_idx + int(np.argmax(self._highs[start_idx:start_idx+initial_period]))
        l_idx = start_idx + int(np.argmin(self._lows[start_idx:start_idx+initial_period]))
        
        if h_idx > l_idx:
            # Low came first, so we were going up to High
            pivots.append(Pivot(
                index=int(l_idx),
                price=float(self._lows[l_idx]),
                log_price=float(self._log_lows[l_idx]),
                type_str="LOW",
                time=self._dates[l_idx]
            ))
            direction = 1
            ...
            pivots.append(Pivot(
                index=int(h_idx),
                price=float(self._highs[h_idx]),
                log_price=float(self._log_highs[h_idx]),
                type_str="HIGH",
                time=self._dates[h_idx]
            ))
            direction = -1
            last_extreme_price = pivots[0].price
            last_extreme_idx = pivots[0].index
        ...
        for t in range(int(last_extreme_idx) + 1, end_idx + 1):
            current_close = float(self._closes[t])
            current_high = float(self._highs[t])
            current_low = float(self._lows[t])
            threshold = float(self._min_move_pcts[t]) * min_move_mult
            ...
            if direction == 1:  # Looking for HIGH
                ...
                if current_low <= last_extreme_price * (1.0 - threshold):
                    pivots.append(Pivot(
                        ...
                        price=float(self._highs[last_extreme_idx]),
                        log_price=float(self._log_highs[last_extreme_idx]),
                        type_str="HIGH",
                        time=self._dates[last_extreme_idx]
                    ))
                    direction = -1
                ...
                if current_high >= last_extreme_price * (1.0 + threshold):
                    pivots.append(Pivot(
                        index=int(last_extreme_idx),
                        price=float(self._lows[last_extreme_idx]),
                        log_price=float(self._log_lows[last_extreme_idx]),
                        type_str="LOW",
                        time=self._dates[last_extreme_idx]
                    ))
                    direction = 1
        ...
        if pivots and pivots[-1].index != end_idx:
            last_close = float(self._closes[end_idx])
            last_type = "HIGH" if pivots[-1].type == "LOW" else "LOW"
            pivots.append(Pivot(
                index=end_idx,
                price=last_close,
                log_price=float(self._log_closes[end_idx]),
                type_str=last_type,
                time=self._dates[end_idx]
            ))
    ...
    def _get_atr_buffer(self, pivot: Pivot, multiplier: float = 0.3) -> float:
        atr = float(self._atrs[pivot.index])
        if self.use_log_scale:
            close = float(self._closes[pivot.index])
    ...
    def score_impulse(self, pivots: List[Pivot]) -> float:
        ...
        if self._rsis is not None:
            try:
                rsi_3 = self._rsis[pivots[3].index]
                rsi_5 = self._rsis[pivots[5].index]
    ...
    def _recursive_subdivide(self, parent_wave: WaveNode, default_min_move: float, depth: int):
        ...
        dynamic_move_mult = max(0.1, (parent_range_pct * 0.20) / np.mean(self._min_move_pcts[start_idx:end_idx]))
    ...
    def get_summary_engine_payload(self, primary_waves: List[WaveNode], alternate_waves: List[List[WaveNode]]) -> Dict[str, Any]:
        def build_count_data(waves: List[WaveNode]):
            ...
            if len(last_block) == 5:
                ...
                if self._macd_hists is not None and not pd.isna(self._macd_hists[pivs[-1].index]):
                    macd_val = self._macd_hists[pivs[-1].index]
