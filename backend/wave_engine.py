...
class DailyElliottWaveEngine:
    ...
    def __init__(self, df: pd.DataFrame, use_log_scale: bool = False):
        ...
        # ⚡ Bolt: Cache NumPy arrays for O(1) access in tight loops instead of using Pandas .iloc
        self._close_vals = self.df['Close'].values
        self._high_vals = self.df['High'].values
        self._low_vals = self.df['Low'].values
        self._log_close_vals = self.df['Log_Close'].values
        self._log_high_vals = self.df['Log_High'].values
        self._log_low_vals = self.df['Log_Low'].values
        self._min_move_pct_vals = self.df['min_move_pct'].values
        self._date_vals = self.df['Date'].values
        self._atr_vals = self.df['ATR'].values
        self._rsi_vals = self.df['RSI'].values if 'RSI' in self.df.columns else None
        self._macd_hist_vals = self.df['MACD_Hist'].values if 'MACD_Hist' in self.df.columns else None
    ...
    def run_adaptive_zigzag(self, min_move_mult: float = 1.0, start_idx: int = 0, end_idx: int = None) -> List[Pivot]:
        ...
        if h_idx > l_idx:
            ...
            pivots.append(Pivot(
                index=int(l_idx),
                price=float(self._low_vals[l_idx]),
                log_price=float(self._log_low_vals[l_idx]),
                type_str="LOW",
                time=self._date_vals[l_idx]
            ))
            direction = 1
            ...
            pivots.append(Pivot(
                index=int(h_idx),
                price=float(self._high_vals[h_idx]),
                log_price=float(self._log_high_vals[h_idx]),
                type_str="HIGH",
                time=self._date_vals[h_idx]
            ))
            direction = -1
            last_extreme_price = pivots[0].price
            last_extreme_idx = pivots[0].index
        ...
        for t in range(int(last_extreme_idx) + 1, end_idx + 1):
            current_close = float(self._close_vals[t])
            current_high = float(self._high_vals[t])
            current_low = float(self._low_vals[t])
            threshold = float(self._min_move_pct_vals[t]) * min_move_mult
            ...
            if direction == 1:  # Looking for HIGH
                ...
                if current_low <= last_extreme_price * (1.0 - threshold):
                    pivots.append(Pivot(
                        ...
                        price=float(self._high_vals[last_extreme_idx]),
                        log_price=float(self._log_high_vals[last_extreme_idx]),
                        type_str="HIGH",
                        time=self._date_vals[last_extreme_idx]
                    ))
                    direction = -1
                ...
                if current_high >= last_extreme_price * (1.0 + threshold):
                    pivots.append(Pivot(
                        index=int(last_extreme_idx),
                        price=float(self._low_vals[last_extreme_idx]),
                        log_price=float(self._log_low_vals[last_extreme_idx]),
                        type_str="LOW",
                        time=self._date_vals[last_extreme_idx]
                    ))
                    direction = 1
        ...
        if pivots and pivots[-1].index != end_idx:
            last_close = float(self._close_vals[end_idx])
            last_type = "HIGH" if pivots[-1].type == "LOW" else "LOW"
            pivots.append(Pivot(
                index=end_idx,
                price=last_close,
                log_price=float(self._log_close_vals[end_idx]),
                type_str=last_type,
                time=self._date_vals[end_idx]
            ))
    ...
    def _get_atr_buffer(self, pivot: Pivot, multiplier: float = 0.3) -> float:
        atr = float(self._atr_vals[pivot.index])
        if self.use_log_scale:
            close = float(self._close_vals[pivot.index])
    ...
    def score_impulse(self, pivots: List[Pivot]) -> float:
        ...
        if 'RSI' in self.df.columns:
            try:
                rsi_3 = self._rsi_vals[pivots[3].index] if self._rsi_vals is not None else np.nan
                rsi_5 = self._rsi_vals[pivots[5].index] if self._rsi_vals is not None else np.nan
    ...
    def _recursive_subdivide(self, parent_wave: WaveNode, default_min_move: float, depth: int):
        ...
        dynamic_move_mult = max(0.1, (parent_range_pct * 0.20) / self._min_move_pct_vals[start_idx:end_idx].mean())
    ...
    def get_trade_recommendation(self, last_block: List[WaveNode]) -> Dict[str, Any]:
        ...
        return {
            ...
            "target": target,
            "invalidation": inval,
            "current_price": float(self._close_vals[-1])
        }
    ...
    def get_summary_engine_payload(self, primary_waves: List[WaveNode], alternate_waves: List[List[WaveNode]]) -> Dict[str, Any]:
        def build_count_data(waves: List[WaveNode]):
            ...
            if len(last_block) == 5:
                ...
                if 'MACD_Hist' in self.df.columns and not pd.isna(self._macd_hist_vals[pivs[-1].index] if self._macd_hist_vals is not None else np.nan):
                    macd_val = self._macd_hist_vals[pivs[-1].index] if self._macd_hist_vals is not None else np.nan
