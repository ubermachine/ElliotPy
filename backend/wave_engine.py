...
class DailyElliottWaveEngine:
    ...
    def run_adaptive_zigzag(self, min_move_mult: float = 1.0, start_idx: int = 0, end_idx: int = None) -> List[Pivot]:
        ...
        # Using numpy arrays for max/min index
        slice_high = self._high_vals[start_idx:start_idx+initial_period]
        slice_low = self._low_vals[start_idx:start_idx+initial_period]

        # argmax/argmin returns index relative to slice, so add start_idx
        h_idx = int(np.nanargmax(slice_high) + start_idx) if len(slice_high) > 0 else start_idx
        l_idx = int(np.nanargmin(slice_low) + start_idx) if len(slice_low) > 0 else start_idx
