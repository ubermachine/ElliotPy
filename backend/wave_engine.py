...
class DailyElliottWaveEngine:
    ...
    def __init__(self, df: pd.DataFrame, use_log_scale: bool = False):
        ...
        # Cache numpy arrays for O(1) array access in tight loops instead of slow pd.iloc
        self._np_close = self.df['Close'].values
        self._np_high = self.df['High'].values
        self._np_low = self.df['Low'].values
        self._np_log_close = self.df['Log_Close'].values
        self._np_log_high = self.df['Log_High'].values
        self._np_log_low = self.df['Log_Low'].values
        self._np_min_move_pct = self.df['min_move_pct'].values
        self._np_date = self.df['Date'].values
        self._np_atr = self.df['ATR'].values
        if 'RSI' in self.df.columns:
            self._np_rsi = self.df['RSI'].values
        if 'MACD_Hist' in self.df.columns:
            self._np_macd_hist = self.df['MACD_Hist'].values
    ...
    def run_adaptive_zigzag(self, min_move_mult: float = 1.0, start_idx: int = 0, end_idx: int = None) -> List[Pivot]:
        ...
        h_idx_rel = np.argmax(self._np_high[start_idx:start_idx+initial_period])
        l_idx_rel = np.argmin(self._np_low[start_idx:start_idx+initial_period])
        h_idx = start_idx + h_idx_rel
        l_idx = start_idx + l_idx_rel
        
        if h_idx > l_idx:
            # Low came first, so we were going up to High
            pivots.append(Pivot(
                index=int(l_idx),
                price=float(self._np_low[l_idx]),
                log_price=float(self._np_log_low[l_idx]),
                type_str="LOW",
                time=self._np_date[l_idx]
            ))
            direction = 1
            ...
            pivots.append(Pivot(
                index=int(h_idx),
                price=float(self._np_high[h_idx]),
                log_price=float(self._np_log_high[h_idx]),
                type_str="HIGH",
                time=self._np_date[h_idx]
            ))
            direction = -1
            last_extreme_price = pivots[0].price
            last_extreme_idx = pivots[0].index
        ...
        for t in range(int(last_extreme_idx) + 1, end_idx + 1):
            current_close = float(self._np_close[t])
            current_high = float(self._np_high[t])
            current_low = float(self._np_low[t])
            threshold = float(self._np_min_move_pct[t]) * min_move_mult
            ...
            if direction == 1:  # Looking for HIGH
                ...
                if current_low <= last_extreme_price * (1.0 - threshold):
                    pivots.append(Pivot(
                        ...
                        price=float(self._np_high[last_extreme_idx]),
                        log_price=float(self._np_log_high[last_extreme_idx]),
                        type_str="HIGH",
                        time=self._np_date[last_extreme_idx]
                    ))
                    direction = -1
                ...
                if current_high >= last_extreme_price * (1.0 + threshold):
                    pivots.append(Pivot(
                        index=int(last_extreme_idx),
                        price=float(self._np_low[last_extreme_idx]),
                        log_price=float(self._np_log_low[last_extreme_idx]),
                        type_str="LOW",
                        time=self._np_date[last_extreme_idx]
                    ))
                    direction = 1
        ...
        if pivots and pivots[-1].index != end_idx:
            last_close = float(self._np_close[end_idx])
            last_type = "HIGH" if pivots[-1].type == "LOW" else "LOW"
            pivots.append(Pivot(
                index=end_idx,
                price=last_close,
                log_price=float(self._np_log_close[end_idx]),
                type_str=last_type,
                time=self._np_date[end_idx]
            ))
    ...
    def _get_atr_buffer(self, pivot: Pivot, multiplier: float = 0.3) -> float:
        atr = float(self._np_atr[pivot.index])
        if self.use_log_scale:
            close = float(self._np_close[pivot.index])
    ...
    def score_impulse(self, pivots: List[Pivot]) -> float:
        ...
        if 'RSI' in self.df.columns:
            try:
                rsi_3 = self._np_rsi[pivots[3].index]
                rsi_5 = self._np_rsi[pivots[5].index]
    ...
    def _recursive_subdivide(self, parent_wave: WaveNode, default_min_move: float, depth: int):
        ...
        dynamic_move_mult = max(0.1, (parent_range_pct * 0.20) / np.mean(self._np_min_move_pct[start_idx:end_idx]))
    ...
    def get_trade_recommendation(self, last_block: List[WaveNode]) -> Dict[str, Any]:
        ...
        return {
            ...
            "target": target,
            "invalidation": inval,
            "current_price": float(self._np_close[-1])
        }
    ...
    def get_summary_engine_payload(self, primary_waves: List[WaveNode], alternate_waves: List[List[WaveNode]]) -> Dict[str, Any]:
        def build_count_data(waves: List[WaveNode]):
            ...
            if len(last_block) == 5:
                ...
                if 'MACD_Hist' in self.df.columns and not pd.isna(self._np_macd_hist[pivs[-1].index]):
                    macd_val = self._np_macd_hist[pivs[-1].index]
