...
class DailyElliottWaveEngine:
    ...
    def __init__(self, df: pd.DataFrame, use_log_scale: bool = False):
        ...
        # Pre-fetch numpy arrays for performance in tight loops
    ...
    def run_adaptive_zigzag(self, min_move_mult: float = 1.0, start_idx: int = 0, end_idx: int = None) -> List[Pivot]:
        ...
        # Fast local extrema using numpy slices
        h_idx_offset = np.argmax(self._high_vals[start_idx:start_idx+initial_period])
        l_idx_offset = np.argmin(self._low_vals[start_idx:start_idx+initial_period])
        h_idx = start_idx + int(h_idx_offset)
        l_idx = start_idx + int(l_idx_offset)
    ...
    def _recursive_subdivide(self, parent_wave: WaveNode, default_min_move: float, depth: int):
        ...
        # Use numpy array for fast mean calculation
        mean_min_move = float(np.mean(self._min_move_pct_vals[start_idx:end_idx]))
        if mean_min_move == 0:
            mean_min_move = 0.001
