...
for i, line in enumerate(lines):
    if "if is_invalidated:" in line and "st.error" in lines[i+1]:
        # Inject tabs
        new_lines.append("tab_ew, tab_inst = st.tabs([\"🌊 Elliott Wave Engine\", \"🏦 Institutional Flow & Profile\"])\n\n")
        new_lines.append("with tab_ew:\n")
        new_lines.append("    " + line)
        in_tab = True
    elif in_tab:
        # Avoid double-indenting if it's already properly indented, but since we are moving it all...
        if line.strip() == "":
            new_lines.append(line)
        else:
            new_lines.append("    " + line)
...
# Now add the institutional tab content at the end
